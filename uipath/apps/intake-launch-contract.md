...
5. `StartTreatmentAccessCase` creates the Data Service case snapshot and calls
   the API Workflow `WriteEventMirror` with the first `case.created` record.
...
1. Validate required fixture IDs and scenario toggles.
2. Generate or accept a human-readable `taccCaseId`, for example
   `TACC-20260629-001`.
3. Upsert the Data Service `TreatmentAccessCase` record with initial status
   `intake_started` and stage `Intake & Hydration`.
4. Call `WriteEventMirror` with the first event record.
5. Return `{ "taccCaseId": "...", "demoRunId": "...", "nextStage": "Intake & Hydration" }`.
...
- Implement `StartTreatmentAccessCase` and `WriteEventMirror` with the payloads
  above.
- `WriteEventMirror` should call the local/event mirror API or Data Service
  event entity used by the Command Center.
